...
function evolvesTo() {
    fetch('https://pokeapi.co/api/v2/pokemon-species/' + input.value)
        ...
        .then((data) => {
            ...
            var evoData = data.evolution_chain.url;
            //console.log(evoData);
        ...
        .then((data) => {
            let base = data.chain.evolves_to[0];
            let emptyArr = [];
            if (base.length == emptyArr.length){
                console.log("computer says no")
            }
            else {
                console.log(base.evolves_to[0].species.name);
            }
        });
//.evolves_to[0]
    /* //name
...
document.getElementById("right-cross-button").addEventListener("click", evolvesTo);
